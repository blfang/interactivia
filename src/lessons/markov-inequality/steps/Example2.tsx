...
export default function Example2({ onCompleteChange }: StepProps) {
  ...
  return (
    <>
      <Markdown>{`
Let's try another example. **Achieve a mean of \\$20 as before, and maximize the number of people with at least \\$90.**
What prevents too many people from having at least \\$90?
      `}</Markdown>
      <MeanWidget
        people={PEOPLE}
        target={ME.mean}
        ...
        solveValues={[0, 0, 0, 10, 90]}
        optimalK={optimal}
      />
      <Markdown>{`
Let's keep track of the examples we've seen so far in a table.
      `}</Markdown>
